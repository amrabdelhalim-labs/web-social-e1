...
صوري تطبيق ويب Full-Stack مبني بـ **Next.js 16** (App Router) و **TypeScript** و **MongoDB**. الواجهة عربية (RTL) باستخدام **MUI 7**. المصادقة عبر **JWT** (موقّع على الخادم) مخزَّن في **Cookie HttpOnly** مع **bcrypt**، وحماية مسارات عبر **Proxy** و`ProtectedRoute`. طبقة البيانات تعتمد **نمط المستودعات**. رفع الصور: استراتيجية تخزين قابلة للتبديل + **التحقق من محتوى الملف (Magic Bytes)** على الخادم. الاختبارات بـ **Vitest** و **Testing Library** (35 ملف اختبار).
