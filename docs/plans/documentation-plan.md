...
| 04  | المصادقة والحماية              | `auth.ts`, `authCookie.ts`, `validators/index.ts`, `auth.middleware.ts`, `api/auth/*` (بما فيها logout), `apiErrors.ts`, `AuthContext.tsx`, `useAuth.ts`, `GuestRoute.tsx`, `ProtectedRoute.tsx`, `src/proxy.ts`                                                                                                                                      |
