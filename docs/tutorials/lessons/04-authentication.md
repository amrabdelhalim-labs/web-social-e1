...
ملف **`src/proxy.ts`** يعمل قبل رندر الصفحة: إن زار مستخدم غير مسجّل مساراتًا محمية (`/my-photos`، `/profile`) بلا cookie، يُعاد توجيهه إلى `/login?next=...`. صفحات الضيف (`/login`، `/register`) تُعاد بعيدًا إن وُجدت جلسة (cookie). التحقق هنا **من وجود الـ cookie** فقط؛ التحقق الكامل من صلاحية الـ JWT يبقى في مسارات API.
