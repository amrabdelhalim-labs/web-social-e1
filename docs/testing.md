...
- يُعرّف `ResizeObserver` وهمي (mock) لتجنب الأخطاء في بيئة jsdom
- يُعرّف `window.matchMedia` وهمي

### 2.3 أدوات مساعدة (`src/app/tests/utils.tsx`)

- `renderWithProviders()` — تُغلّف المكونات بـ AuthContext و ThemeContext
- `createMockUser()` — ينشئ مستخدمًا وهميًا بخصائص قابلة للتخصيص
- `createMockPhoto()` — ينشئ صورة وهمية
...
| الأمر | الوصف |
|-------|-------|
| `npm test` | وضع المراقبة — يُعيد تشغيل الاختبارات عند التعديل |
| `npm run test -- --run` | تشغيل واحد وإنهاء |
| `npm run test -- --run --reporter=verbose` | تشغيل مع تفاصيل كل حالة |
| `npm run test -- --run src/app/tests/auth.test.ts` | تشغيل ملف محدد |
...
it('displays user name when logged in', () => {
  const user = createMockUser({ name: 'أحمد' });
...
*للمعمارية التقنية، راجع [docs/ai/architecture.md](ai/architecture.md).*
*لمرجع المستودعات، راجع [docs/repository-quick-reference.md](repository-quick-reference.md).*
